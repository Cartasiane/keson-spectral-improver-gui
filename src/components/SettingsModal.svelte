...
<div class="modal">
  ...
  <div class="settings-grid">
    <label>
      ...
      <small class="muted">Sous ce débit, le fichier est signalé comme low.</small>
      <input
        type="number"
        min="32"
        max="1000"
    ...
    <label>
      ...
      <input
        ...
        on:input={(e) => updateSetting('analysis_window_seconds', Number(e.target.value))}
      />
    </label>
    <label>
    ...
    <label>
      ...
      <input
        ...
        on:input={(e) => updateSetting('cache_max_entries', Number(e.target.value))}
      />
    </label>
  </div>
  {#if message}
  ...
  <div class="actions" style="justify-content:flex-end; gap:8px; margin-top:10px;">
    ...
    <button class="btn primary" disabled={loading} on:click={() => dispatch('save')}>
      {loading ? '...' : 'Enregistrer'}
    </button>
  </div>
</div>
